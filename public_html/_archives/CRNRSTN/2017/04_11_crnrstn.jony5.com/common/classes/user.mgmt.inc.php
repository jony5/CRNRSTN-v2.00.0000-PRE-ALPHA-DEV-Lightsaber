...
class user {
	...
	private static $accntStatus;
	private static $accntQuery_search;
	private static $accntQuery_FILTER_LOCKED;
	private static $accntQuery_FILTER_PUBLICNOTE;
	private static $accntQuery_FILTER_PUBLISHME;
	private static $accntQuery_FILTER_NOTES;
	private static $accntQuery_FILTER_DELETEDACCNT;
	private static $accntQuery_FILTER_CENSOREDACCNT;
	private static $accntQuery_FILTER_CENSOREDNOTE;
	private static $accntQuery_FILTER_LIKES;
	private static $accntQuery_FILTER_REPLIEDTO;
	private static $accntQuery_FILTER_REPLIES;
	private static $accntQuery_FILTER_CODE;
	private static $accntQuery_FILTER_LOGGEDIN;
	private static $accntQuery_FILTER_USRDELETED;
	private static $commSource;
	private static $FBQuery_search;
	private static $FBQuery_COMM_FB_FILTER_SPAM;
	private static $FBQuery_COMM_FB_FILTER_OPTIN;
	private static $FBQuery_COMM_FB_FILTER_GENCOMM;
	private static $FBQuery_COMM_FB_FILTER_GENQUEST;
	private static $FBQuery_COMM_FB_FILTER_FEATREQ;
	private static $FBQuery_COMM_FB_FILTER_BUGREPORT;
	private static $FBQuery_COMM_FB_FILTER_RESPONDED;
	...
	public function updateContent_CRNRSTN($contentType){
		...
		switch($contentType){
			...
			case 'edit_techspec':
			case 'edit_method':
				#error_log("/crnrstn/ user.mgmt.inc.php (609) edit_method");
				//
				// GET FORM DATA
	...
	public function logout($mode=NULL){
		...
		session_destroy();
		unset($oCOOKIE_MGR);
		header("Location: ".$this->getEnvParam('ROOT_PATH_CLIENT_HTTP').$this->getEnvParam('ROOT_PATH_CLIENT_HTTP')."/".$tmp_param);
		exit();
	...
	public function retrieveCrnstnAccntInfo(){
		if(self::$oUserEnvironment->oHTTP_MGR->issetHTTP($_POST)){
			self::$accntStatus = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'ACCOUNT_STATUS');
			self::$timeSpan = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'TIMESPAN');
			self::$accntQuery_search = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'usradmin_t');
			self::$accntQuery_FILTER_LOCKED = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_LOCKED');
			self::$accntQuery_FILTER_PUBLICNOTE = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_PUBLICNOTE');
			self::$accntQuery_FILTER_PUBLISHME = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_PUBLISHME');
			self::$accntQuery_FILTER_NOTES = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_NOTES');
			self::$accntQuery_FILTER_DELETEDACCNT = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_DELETEDACCNT');			
			self::$accntQuery_FILTER_CENSOREDACCNT = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_CENSOREDACCNT');
			self::$accntQuery_FILTER_CENSOREDNOTE = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_CENSOREDNOTE');
			self::$accntQuery_FILTER_LIKES = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_LIKES');
			self::$accntQuery_FILTER_REPLIEDTO = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_REPLIEDTO');
			self::$accntQuery_FILTER_REPLIES = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_REPLIES');
			self::$accntQuery_FILTER_CODE = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_CODE');
			self::$accntQuery_FILTER_LOGGEDIN = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_LOGGEDIN');
			self::$accntQuery_FILTER_USRDELETED = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'FILTER_USRDELETED');
			
		}else{
			if(self::$oUserEnvironment->oSESSION_MGR->issetSessionParam('TIMESPAN')){
				self::$accntStatus = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('ACCOUNT_STATUS');
				self::$timeSpan = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('TIMESPAN');
				self::$accntQuery_search = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('SEARCH');			
				self::$accntQuery_FILTER_LOCKED = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_LOCKED');
				self::$accntQuery_FILTER_PUBLICNOTE = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_PUBLICNOTE');
				self::$accntQuery_FILTER_PUBLISHME = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_PUBLISHME');
				self::$accntQuery_FILTER_CENSOREDACCNT = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_CENSOREDACCNT');
				self::$accntQuery_FILTER_DELETEDACCNT = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_DELETEDACCNT');
				self::$accntQuery_FILTER_CENSOREDNOTE = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_CENSOREDNOTE');
				self::$accntQuery_FILTER_NOTES = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_NOTES');
				self::$accntQuery_FILTER_LIKES = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_LIKES');
				self::$accntQuery_FILTER_REPLIEDTO = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_REPLIEDTO');
				self::$accntQuery_FILTER_REPLIES = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_REPLIES');
				self::$accntQuery_FILTER_CODE = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_CODE');
				self::$accntQuery_FILTER_LOGGEDIN = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_LOGGEDIN');
				self::$accntQuery_FILTER_USRDELETED = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('FILTER_USRDELETED');
			}else{
				self::$accntStatus = 'PENDING';
				self::$timeSpan = '24HRS';
			}
		}		
		
		return $this->getAccntInfo();
	}
	
	public function retrieveCrnstnCommInfo(){
		if(self::$oUserEnvironment->oHTTP_MGR->issetHTTP($_POST)){
			self::$commSource = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_SOURCE');
			self::$timeSpan = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'TIMESPAN');
			self::$FBQuery_search = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'comm_t');
			self::$FBQuery_COMM_FB_FILTER_SPAM = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_FILTER_SPAM');
			self::$FBQuery_COMM_FB_FILTER_OPTIN = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_FILTER_OPTIN');
			self::$FBQuery_COMM_FB_FILTER_GENCOMM = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_FILTER_GENCOMM');
			self::$FBQuery_COMM_FB_FILTER_GENQUEST = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_FILTER_GENQUEST');
			self::$FBQuery_COMM_FB_FILTER_FEATREQ = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_FILTER_FEATREQ');			
			self::$FBQuery_COMM_FB_FILTER_BUGREPORT = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_FILTER_BUGREPORT');
			self::$FBQuery_COMM_FB_FILTER_RESPONDED = self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'COMM_FB_FILTER_RESPONDED');

		}else{
			if(self::$oUserEnvironment->oSESSION_MGR->issetSessionParam('TIMESPAN')){
				self::$commSource = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_SOURCE');
				self::$timeSpan = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('TIMESPAN');
				self::$FBQuery_search = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('SEARCH');
				self::$FBQuery_COMM_FB_FILTER_SPAM = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_FILTER_SPAM');
				self::$FBQuery_COMM_FB_FILTER_OPTIN = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_FILTER_OPTIN');
				self::$FBQuery_COMM_FB_FILTER_GENCOMM = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_FILTER_GENCOMM');
				self::$FBQuery_COMM_FB_FILTER_GENQUEST = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_FILTER_GENQUEST');
				self::$FBQuery_COMM_FB_FILTER_FEATREQ = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_FILTER_FEATREQ');
				self::$FBQuery_COMM_FB_FILTER_BUGREPORT = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_FILTER_BUGREPORT');
				self::$FBQuery_COMM_FB_FILTER_RESPONDED = self::$oUserEnvironment->oSESSION_MGR->getSessionParam('COMM_FB_FILTER_RESPONDED');
	...
	public function fatClientUGCRefresh($ELEMENTID_SOURCE){
		//
		// PUBLISH UGC NOTES FOR CRNRSTN ELEMENT
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('ELEMENTID_SOURCE' => $ELEMENTID_SOURCE)
		);
		
		self::$methodName = 'fatClientUGCRefresh';
		
		//
		// SEND/RETURN WEB SERVICES NEW COMMENT INSERT STATUS REQUEST
		$tmp_ugc_ARRAY = array();
		$tmp_ugc_ARRAY = self::$soapManager->returnContent(self::$methodName,self::$params);
		
		//
		// BUILD XML/HTML
		// INITIALIZE PARAMETERS FOR XML/HTML BURN
		$tmp_pagination_cnt = ceil((sizeof($tmp_ugc_ARRAY)/$this->getEnvParam('PAGE_INDEXSIZE')));
		$tmp_pageindex = 1;
		$indexcnt = 2;
		$FILEPATH_XML = $this->getEnvParam('DOCUMENT_ROOT').$this->getEnvParam('DOCUMENT_ROOT_DIR').'/common/xml/ugcnotes/';
		$FILEPATH_HTML = $this->getEnvParam('DOCUMENT_ROOT').$this->getEnvParam('DOCUMENT_ROOT_DIR').'/common/html/ugcnotes/';
		...
		for($i=0;$i<sizeof($tmp_ugc_ARRAY);$i++){
			
			$FILENAME_XML = 'crnrstn_'.$ELEMENTID_SOURCE.'_'.$tmp_pageindex.'.xml';
		
			$ugc_notes_xml_output[1] = '<crnrstn_ugc_pi>'.$tmp_pageindex.'</crnrstn_ugc_pi><crnrstn_ugc_indexsize>'.$this->getEnvParam('PAGE_INDEXSIZE').'</crnrstn_ugc_indexsize><crnrstn_ugc_indexcount>'.$tmp_pagination_cnt.'</crnrstn_ugc_indexcount>';
			#$tmp_indexStart = $this->getEnvParam('PAGE_INDEXSIZE')*($ugc_pi);
			
			$FILENAME_HTML = 'crnrstn_'.$tmp_ugc_ARRAY[$i]['NOTEID_SOURCE'].'.html';
			$ugc_notes_html_output[1] = $tmp_ugc_ARRAY[$i]['NOTE_STYLED'];
			//error_log('(2690) :: '.$ugc_notes_html_output[1]);
			
			if($tmp_ugc_ARRAY[$i]['NOTE_ELEM_TT']!=''){
				$ugc_notes_html_output[1] .= '<div class="cb_5"></div><div class="comment_tt_wrapper">'.$tmp_ugc_ARRAY[$i]['NOTE_ELEM_TT'].'</div>';
			}
	
			//
			// CONCAT COMMENT XML OUTPUT
			$ugc_notes_xml_output[3] .= '<crnrstn_comment><crnrstn_username_disp>'.$this->XML_sanitize($tmp_ugc_ARRAY[$i]['USERNAME_DISPLAY']).'</crnrstn_username_disp><crnrstn_external_uri>'.$this->XML_sanitize($tmp_ugc_ARRAY[$i]['EXTERNAL_URI_FORMATTED']).'</crnrstn_external_uri><crnrstn_usr_thumbnail width="'.$tmp_ugc_ARRAY[$i]['IMAGE_WIDTH'].'" height="'.$tmp_ugc_ARRAY[$i]['IMAGE_HEIGHT'].'">'.$tmp_ugc_ARRAY[$i]['IMAGE_NAME'].'</crnrstn_usr_thumbnail><crnrstn_comment_datecreated>'.date("M. j, Y Hi\h\\r\s T", strtotime($tmp_ugc_ARRAY[$i]['DATECREATED'])).'</crnrstn_comment_datecreated><crnrstn_comment_subject>'.$this->XML_sanitize($tmp_ugc_ARRAY[$i]['SUBJECT']).'</crnrstn_comment_subject><crnrstn_usr_comment uri="'.$this->getEnvParam('ROOT_PATH_CLIENT_HTTP').$this->getEnvParam('ROOT_PATH_CLIENT_HTTP_DIR').'common/html/ugcnotes/crnrstn_'.$tmp_ugc_ARRAY[$i]['NOTEID_SOURCE'].'.html">'.$tmp_ugc_ARRAY[$i]['NOTEID_SOURCE'].'</crnrstn_usr_comment></crnrstn_comment>';
...
//				error_log('(2705) :: WRITING COMMENT '.($i+1).' TO HTML FILE ::<br>
//				----PATH='.$FILEPATH_HTML.'<br>
//				----FILE='.$FILENAME_HTML.'<br> 
//				----SIZE='.strlen($tmp_html_output));
			...
			if($indexcnt>$this->getEnvParam('PAGE_INDEXSIZE')){
				$tmp_pageindex++;
				$indexcnt=2;
...
//					error_log('(2726) :: WRITING XML FILE ::<br>
//					----PATH='.$FILEPATH_XML.'<br>
//					----FILE='.$FILENAME_XML.'<br>
//					----SIZE='.strlen($tmp_xml_output));
...
//			----SIZE='.strlen($tmp_xml_output));
	...
	public function retrieveMessages(){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oGetMessagesRequest' => 'true');

		self::$methodName = 'retrieveMessages';
	...
	private function updateMessage(){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oEditMessageSubmission' =>
			array('MSG_KEYID' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgkey'),
			'MSG_KEYID_CRC32' => crc32(self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgkey')),
			'MSG_NAME' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgname'),
			'MSG_SUBJECT' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgsubject'),
			'MSG_HTML' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msghtml'),
			'MSG_TEXT' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgtext'),
			'MSG_DESCRIPTION' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgdescription')
		));
	...
	private function submitNewMessage(){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oNewMessageSubmission' =>
			array('MSG_KEYID' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgkey'),
			'MSG_KEYID_CRC32' => crc32(self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgkey')),
			'MSG_NAME' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgname'),
			'MSG_SUBJECT' => self::$oUserEnvironment->oHTTP_MGR->extractData($_POST,'msgsubject'),
		...
		self::$methodName = 'submitNewMessage';
	...
	public function getCommInsertStatus(){
		...
		self::$params = array('oCommentInsertStatus' =>
			array(
			'USER' => $tmp_user,
			'METHODID_SOURCE' => $this->methodID_SOURCE, 
			'CLASSID_SOURCE' => $this->classID_SOURCE
			)
		);
	...
	public function getUserCommentbyUN($un){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		$tmp_user = 
			array('USERNAME' => $un
		...
		self::$params = array('oCommentRequestbyUN' =>
			array('USER' => $tmp_user
			)
		);
		
		self::$methodName = 'getUserCommentbyUN';
	...
	public function getFeedbackbyID($nid){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oCommFBRequest' =>
			array('FID_SOURCE' => $nid
			)
		);
		
		self::$methodName = 'getFeedbackbyID';
	...
	private function getAccntInfo(){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountInfoRequest' =>
			array('ACCOUNT_STATUS' => self::$accntStatus, 
			'TIMESPAN' => self::$timeSpan, 
			'FILTER_LOCKED' => self::$accntQuery_FILTER_LOCKED, 
			'FILTER_PUBLICNOTE' => self::$accntQuery_FILTER_PUBLICNOTE, 
			'FILTER_PUBLISHME' => self::$accntQuery_FILTER_PUBLISHME, 
			'FILTER_NOTES' => self::$accntQuery_FILTER_NOTES,
			'FILTER_DELETEDACCNT' => self::$accntQuery_FILTER_DELETEDACCNT,
			'FILTER_CENSOREDACCNT' => self::$accntQuery_FILTER_CENSOREDACCNT, 
			'FILTER_CENSOREDNOTE' => self::$accntQuery_FILTER_CENSOREDNOTE, 
			'FILTER_LIKES' => self::$accntQuery_FILTER_LIKES, 
			'FILTER_REPLIEDTO' => self::$accntQuery_FILTER_REPLIEDTO, 
			'FILTER_REPLIES' => self::$accntQuery_FILTER_REPLIES, 
			'FILTER_CODE' => self::$accntQuery_FILTER_CODE, 
			'FILTER_LOGGEDIN' => self::$accntQuery_FILTER_LOGGEDIN,
			'FILTER_USRDELETED' => self::$accntQuery_FILTER_USRDELETED,
			'SEARCH_PARAM' => self::$accntQuery_search, 
			'SEARCH_PARAM_SEARCH' => $this->search_FillerSanitize(self::$accntQuery_search)
			)
		);
			
		self::$methodName = 'getAccntInfo';
	...
	private function getCommInfo(){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oCommFBRequest' =>
			array('FEEDBACK_SOURCE' => self::$commSource,
			'TIMESPAN' => self::$timeSpan,
			'COMM_FB_FILTER_SPAM' => self::$FBQuery_COMM_FB_FILTER_SPAM,
			'COMM_FB_FILTER_OPTIN' => self::$FBQuery_COMM_FB_FILTER_OPTIN,
			'COMM_FB_FILTER_GENCOMM' => self::$FBQuery_COMM_FB_FILTER_GENCOMM,
			'COMM_FB_FILTER_GENQUEST' => self::$FBQuery_COMM_FB_FILTER_GENQUEST,
			'COMM_FB_FILTER_FEATREQ' => self::$FBQuery_COMM_FB_FILTER_FEATREQ,
			'COMM_FB_FILTER_BUGREPORT' => self::$FBQuery_COMM_FB_FILTER_BUGREPORT,
			'COMM_FB_FILTER_RESPONDED' => self::$FBQuery_COMM_FB_FILTER_RESPONDED,
			'SEARCH_PARAM' => self::$FBQuery_search,
			'SEARCH_PARAM_SEARCH' => $this->search_FillerSanitize(self::$FBQuery_search)
			)
		);
			
		self::$methodName = 'getCommInfo';
	...
	public function lockAccnt($USERNAME){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
	...
	public function unlockAccnt($USERNAME){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME
	...
	public function censorAccnt($USERNAME){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME
	...
	public function uncensorAccnt($USERNAME){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME
	...
	public function deleteAccnt($USERNAME){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME
	...
	public function restoreAccnt($USERNAME){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME
	...
	public function censorAccntNote($USERNAME,$NOTEID_SOURCE,$ELEMENTID_SOURCE){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME,
			'NOTEID_SOURCE' => $NOTEID_SOURCE,
			'ELEMENTID_SOURCE' => $ELEMENTID_SOURCE
			)
		);
			
		self::$methodName = 'censorAccntNote';
	...
	public function uncensorAccntNote($USERNAME,$NOTEID_SOURCE,$ELEMENTID_SOURCE){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME,
		...
		self::$methodName = 'uncensorAccntNote';
	...
	public function publishAccntNote($USERNAME,$NOTEID_SOURCE,$ELEMENTID_SOURCE){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME,
		...
		self::$methodName = 'publishAccntNote';
	...
	public function unpublishAccntNote($USERNAME,$NOTEID_SOURCE,$ELEMENTID_SOURCE){
		//
		// INITIALIZE PARAMS FOR SOAP OBJECT REQUEST
		self::$params = array('oAccountMgmtRequest' =>
			array('USERNAME' => $USERNAME,
		...
		self::$methodName = 'unpublishAccntNote';
	...
	public function transactionStatusUpdate($statusCode,$statusSource){
		...
		switch($statusSource){
			...
			break;
			case 'delete_usr':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The user was successfully deleted.','error'=>'Error :: Oops...an error was experienced while deleting that user.<br>Please try again later.');
			break;
			case 'delete_usr_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while deleting that user.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'restore_usr':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The user was successfully restored.','error'=>'Error :: Oops...an error was experienced while restoring that user.<br>Please try again later.');
			break;
			case 'restore_usr_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while restoring that user.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'lock_usr':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The user account was successfully locked.','error'=>'Error :: Oops...an error was experienced while locking that user account.<br>Please try again later.');
			break;
			case 'lock_usr_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while locking that user account.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'unlock_usr':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The user account was successfully unlocked.','error'=>'Error :: Oops...an error was experienced while unlocking that user account.<br>Please try again later.');
			break;
			case 'unlock_usr_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while unlocking that user account.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'censor_usr':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The user account was successfully censored.','error'=>'Error :: Oops...an error was experienced while censoring that user account.<br>Please try again later.');
			break;
			case 'censor_usr_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while censoring that user account.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'uncensor_usr':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: Censorship was successfully removed from that user account.','error'=>'Error :: Oops...an error was experienced while uncensoring that user account.<br>Please try again later.');
			break;
			case 'uncensor_usr_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while uncensoring that user account.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'censor_usr_note':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The user note was successfully censored.','error'=>'Error :: Oops...an error was experienced while censoring that user note.<br>Please try again later.');
			break;
			case 'censor_usr_note_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while censoring that user note.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'uncensor_usr_note':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: Censorship was successfully removed from that user note.','error'=>'Error :: Oops...an error was experienced while uncensoring that user note.<br>Please try again later.');
			break;
			case 'uncensor_usr_note_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while uncensoring that user note.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'publish_usr_note':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: That user note was successfully published.','error'=>'Error :: Oops...an error was experienced while publishing that user note.<br>Please try again later.');
			break;
			case 'publish_usr_note_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while publishing that user note.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'unpublish_usr_note':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: That user note was successfully unpublished.','error'=>'Error :: Oops...an error was experienced while unpublishing that user note.<br>Please try again later.');
			break;
			case 'unpublish_usr_note_fall':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'','error'=>'Error :: Oops...an error was experienced while unpublishing that user note.<br>Please try again later.<br>No updates were able to be made.');
			break;
			case 'new_message':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The new message has been added. Thanks!','error'=>'Error :: It looks like there is a problem adding the new message.');
			break;
			case 'edit_message':
				$tmp_msg_ARRAY[$statusSource] = array('success'=>'Success :: The message has been updated. Thanks!','error'=>'Error :: It looks like there was a problem updating the selected message.');
			break;
		}
