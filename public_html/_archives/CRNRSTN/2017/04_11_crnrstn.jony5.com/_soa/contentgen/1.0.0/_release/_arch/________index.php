		...
		<div id="content_area_main">
			...
			<div id="doc_content_results_wrapper">
				<div id="doc_content_results">
					<h1 id="content_results_title"><?php if($oUSER->getEnvParam('CONTENT_MODE')=='SOAP'){echo $oUSER->contentOutput_ARRAY[1]['NAME'].$classDesignation;} ?></h1>
					<div class="cb_15"></div>
					<div id="content_results_body">
						...
						<div class="cb"></div>
						<div class="content_results_subtitle_divider"></div>
						<p id="xhandle_description"><?php if($oUSER->getEnvParam('CONTENT_MODE')=='SOAP'){echo $oUSER->contentOutput_ARRAY[1]['DESCRIPTION'];} ?></p>
						...
						<ul>
						<?php
							if($oUSER->getEnvParam('CONTENT_MODE')=='SOAP'){
						...
						<div class="cb"></div>
						<div class="content_results_subtitle_divider"></div>
						<p id="xhandle_invokingclass"><?php if($oUSER->getEnvParam('CONTENT_MODE')=='SOAP'){echo $oUSER->contentOutput_ARRAY[1]['CLASSNAME'];} ?></p>
						...
						<p id="xhandle_methoddefine"><?php if($oUSER->getEnvParam('CONTENT_MODE')=='SOAP'){echo $oUSER->contentOutput_ARRAY[1]['METHODDEFINE'];} ?></p>
						<?php if(sizeof($oUSER->contentOutput_ARRAY[1]['PARAMETERS'])>0 || $oUSER->getUserParam('USER_PERMISSIONS_ID')>399){ ?>
						<div class="cb_15"></div>
						...
						<div class="cb"></div>
						<div class="content_results_subtitle_divider"></div>
						<p id="xhandle_returnedvalue"><?php if($oUSER->getEnvParam('CONTENT_MODE')=='SOAP'){echo $oUSER->contentOutput_ARRAY[1]['RETURNEDVALUE'];} ?></p>
						...
						<div class="cb"></div>
						<div class="content_results_subtitle_divider"></div>
						<p id="xhandle_currentversion"><?php if($oUSER->getEnvParam('CONTENT_MODE')=='SOAP'){echo $oUSER->contentOutput_ARRAY[1]['VERSION'];} ?></p>
						...
						<?php	
							...
							if($oUSER->getUserParam('USER_PERMISSIONS_ID')>399){
								for($i=0;$i<sizeof($oUSER->contentOutput_ARRAY[1]['EXAMPLES']);$i++){
								?>
								<div class="editlnk_editable_section" style="margin:0 10px 0 10px; float:left;" onClick="mycrnrstn_fhandler.initAdminForm('edit_example','edit_example','<?php echo $oUSER->getEnvParam('ROOT_PATH_CLIENT_HTTP').$oUSER->getEnvParam('ROOT_PATH_CLIENT_HTTP_DIR').'admin/mgmt/_frms/examples_edit.php?'.$contentParam.'='.$contentID.'&e='.$oUSER->contentOutput_ARRAY[1]['EXAMPLES'][$i]['EXAMPLEID'].'&uri='.urlencode($oUSER->contentOutput_ARRAY[1]['URI']); ?>'); return false;">edit example <?php echo $i+1; ?></div>
								<?php
								}
						...
						?>
						<div class="cb_15"></div>
						<div class="title_editable_section"><h3 class="content_results_subtitle">Add Comment ::</h3></div>
