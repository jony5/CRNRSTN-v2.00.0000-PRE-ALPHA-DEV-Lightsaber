...
class database_integration {
	...
	private function dbQuery($queryType, $oUserEnvironment, $oUser){
		try{
			error_log('/services/ database.inc.php (136) queryType sent to user database_integrations class object :: '.$queryType);
...
//						';
				...
				case 'svc_getAllToolTips':
					...
					#error_log("/services/ database.inc.php (812) ".self::$query_elements);
					//
					// PROCESS QUERY
				...
				case 'svc_isUserUnique':
					...
					#error_log('services database.inc.php (858) svc_isUserUnique :: '.self::$query);
				...
				case 'svc_creatNewUser':
					...
					//
					// PREPARE QUERY
					self::$query = 'INSERT INTO `users` (`USERNAME`,`USERID_SOURCE`,`USERNAME_CRC32`,`USERID`,`FIRSTNAME`,`LASTNAME`,`USERNAME_DISPLAY`,`EMAIL`,`EMAIL_CRC32`,`PWDHASH`,
														 `LASTLOGIN`,`SESSION_PERSIST`,`DATEMODIFIED`) VALUES ("'.$mysqli->real_escape_string(strtolower($oUser->getReqParamByKey('USERNAME'))).'",
														 "'.$seednum_full.'","'.crc32(strtolower($oUser->getReqParamByKey('USERNAME'))).'","'.crc32($seednum_full).'",
														 "'.$oUser->getReqParamByKey('FIRSTNAME').'","'.$oUser->getReqParamByKey('LASTNAME').'",
														 "'.$oUser->getReqParamByKey('USERNAME').'","'.strtolower($oUser->getReqParamByKey('EMAIL')).'",
														 "'.crc32(strtolower($oUser->getReqParamByKey('EMAIL'))).'","'.$oUser->getReqParamByKey('PWDHASH').'","'.$ts.'",
														 "'.$oUser->getReqParamByKey('SESSION_PERSIST').'","'.$ts.'");';
					...
					self::$query .= 'INSERT INTO `crnrstn_stage`.`msg_queue` (`MSG_SOURCEID`,`MSG_SOURCEID_CRC32`,`MSG_KEYID`,`MSG_KEYID_CRC32`,`EMAIL`,
					...
					#error_log('/services/ database.inc.php (926) svc_creatNewUser :: '.self::$query);
				...
				case 'svc_activateNewUser':
					...
					#error_log('/services/ database.inc.php (982) svc_activateNewUser query :: '.self::$query);
					
					//
					// PROCESS QUERY
					self::$result = $oUserEnvironment->oMYSQLI_CONN_MGR->processQuery($mysqli, self::$query);
					#error_log("/services/ database.inc.php (994) Num Rows: ".self::$result->num_rows);
					switch(self::$result->num_rows){
						case 1:
							...
							while ($row = self::$result->fetch_row()) {
								...
								#error_log("/services/ database.inc.php (994) USER TO ACTIVATE SETTING: ".$row[self::$queryDescript_ARRAY['users_activation_USERNAME']]);
								switch($row[self::$queryDescript_ARRAY['users_activation_ISACTIVE']]){
									case 0:
										//
										// ACTIVATE ACCOUNT. BUILD QUERIES
										self::$query = 'UPDATE `users` SET `ISACTIVE`="1",`DATEMODIFIED`="'.$ts.'" 
										WHERE `USERNAME`="'.$row[self::$queryDescript_ARRAY['users_activation_USERNAME']].'" AND 
										`USERID_SOURCE`="'.md5($row[self::$queryDescript_ARRAY['users_activation_USERNAME']]).'" AND 
										`USERNAME_CRC32`="'.crc32($row[self::$queryDescript_ARRAY['users_activation_USERNAME']]).'" AND
										`USERID`="'.crc32(md5($row[self::$queryDescript_ARRAY['users_activation_USERNAME']])).'" LIMIT 1;';
										
										self::$query .= 'UPDATE `users_activation` SET `ISACTIVE`="1",`DATEMODIFIED`="'.$ts.'" 
										WHERE `KEY`="'.$row[self::$queryDescript_ARRAY['users_activation_KEY']].'" AND 
										`KEY_CRC32`="'.$row[self::$queryDescript_ARRAY['users_activation_KEY_CRC32']].'" AND 
										`USERNAME`="'.$row[self::$queryDescript_ARRAY['users_activation_USERNAME']].'" AND
										`USERNAME_CRC32`="'.$row[self::$queryDescript_ARRAY['users_activation_USERNAME_CRC32']].'" LIMIT 1;';
										
										#error_log('/services/ database.inc.php (1010) :: '.self::$query);
										//
										// PROCESS QUERY
										self::$result = $oUserEnvironment->oMYSQLI_CONN_MGR->processMultiQuery($mysqli, self::$query);
										...
										// CLOSE CONNECTION
										$oUserEnvironment->oMYSQLI_CONN_MGR->closeConnection($mysqli);					
										return 'accountactivate=true&persist='.$row[self::$queryDescript_ARRAY['users_SESSION_PERSIST']];
								
									break;
									default:
										//
										// ACCOUNT IS ALREADY ACTIVATED. NO FURTHER UPDATES NEEDED. SAVE THE RESOURCES
										return 'accountactivate=donealready&persist='.$row[self::$queryDescript_ARRAY['users_SESSION_PERSIST']];
									break;
								}
						...
						case 0:
							...
							#error_log('(1037) :: ACTIVATION ERROR :: NUMROW=0|accountactivate=dataerror_null|');
							return 'accountactivate=dataerror_null';
						break;
						default:
							//
							// POTENTIAL SYSTEM ERR
							#error_log('(1043) :: ACTIVATION ERROR :: NUMROW='.self::$result->num_rows.'|accountactivate=dataerror_redun|');
				...
				case 'svc_updateUserProfile':
					...
					if(strlen($oUser->getReqParamByKey('IMAGE_NAME'))>10){
						...
						if(strlen($oUser->getReqParamByKey('ELEMENTID_PIPED'))>10){
							...
							foreach($tmp_elementid_ARRAY as $key=>$val){
								$dyn_tbl_users = 'crnrstn_'.$mysqli->real_escape_string($val).'_users';
								self::$query .= 'UPDATE `'.$dyn_tbl_users.'` SET `EXTERNAL_URI_FORMATTED`="'.$oUser->getReqParamByKey('EXTERNAL_URI_FORMATTED').'",`DATEMODIFIED`="'.$ts.'" WHERE `USERNAME`="'.$mysqli->real_escape_string($oUser->getReqParamByKey('USERNAME')).'" AND `USERID_SOURCE`="'.$oUser->getReqParamByKey('USERID_SOURCE').'" AND `USERNAME_CRC32`="'.crc32($oUser->getReqParamByKey('USERNAME')).'" AND `USERID`="'.crc32($oUser->getReqParamByKey('USERID_SOURCE')).'" LIMIT 1;';
								#self::$result = $oUserEnvironment->oMYSQLI_CONN_MGR->processQuery($mysqli, self::$query);
							}
						}
					}
...
//					REMOTE_ADDR = 							INET_ATON() & INET_NTOA()
				...
				case 'svc_postUserComment':					
					...
					"'.$oUser->returnCommentParam('SUBJECT').'",
					...
					if($oUser->returnCommentParam('PUBLISH_ME')=='1'){
						...
						"'.$this->search_FillerSanitize($tmp_feedback).'","0","1","1","0","0","1",
...
//						// ROLL BACK DUE TO ERROR
				...
				case 'svc_updateUserComment':
					...
					self::$query .= 'UPDATE `user_notes` SET `SUBJECT`="'.$oUser->returnCommentParam('SUBJECT').'",
					...
					if($oUser->returnCommentParam('PUBLISH_ME')=='1'){
						...
						"'.$this->search_FillerSanitize($tmp_feedback).'",
...
//						// ROLL BACK DUE TO ERROR
				...
				case 'svc_submitExamples':
					error_log("/services/ database.inc.php (1658) processing svc_submitExamples");
					//
					// BUILD QUERY
					if($oUser->getReqParamByKey('EXAMPLEID')!=''){
						...
						if($oUser->getReqParamByKey('EXAMPLE_RAW')!=''){
							...
							//
							// DELETE EXAMPLE AS THERE IS NO EXAMPLE CONTENT
					...
					#error_log("/services/ database.inc.php (1692) svc_submitExamples query: ".self::$query);
					//
					// PROCESS QUERY
					self::$result = $oUserEnvironment->oMYSQLI_CONN_MGR->processMultiQuery($mysqli, self::$query);
					if($mysqli->error){
				...
				break;
				case 'edit_method':
					#error_log("/services/ database.inc.php (1793) edit_method");
					//
					// BUILD QUERY
					if($oUserEnvironment->oHTTP_MGR->issetParam($_POST,'m')){
