...
class user {
	...
	private function getAccntInfo(){
		...
		self::$params = array('oAccountInfoRequest' =>
			array('ACCOUNT_STATUS' => self::$feedback_NAME, 
			'TIMESPAN' => strtolower(self::$feedback_EMAIL), 
			'FILTER_LOCKED' => self::$feedback_FEEDBACK, 
			'FILTER_OPTIN' => $this->search_FillerSanitize(strtolower(self::$feedback_FEEDBACK_SEARCH)), 
			'FILTER_CENSORED' => self::$feedback_FB_BUGREPORT, 
			'FILTER_LIKES' => self::$feedback_FB_FEATREQUEST, 
			'FILTER_REPLIEDTO' => self::$feedback_FB_GENQUESTION, 
			'FILTER_REPLIES' => self::$feedback_FB_GENCOMMENT, 
			'FILTER_CODE' => self::$feedback_OPTIN, 
			'FILTER_LOGGEDIN' => self::$feedback_USERNAME, 
			'SEARCH_PARAM' => self::$feedback_CLASSID_SOURCE, 
			'SEARCH_PARAM_SEARCH' => $this->search_FillerSanitize($s_params)
			)
		);
