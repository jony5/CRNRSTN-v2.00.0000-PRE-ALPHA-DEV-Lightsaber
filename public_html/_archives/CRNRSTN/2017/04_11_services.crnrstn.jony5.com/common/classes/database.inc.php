...
class database_integration {
	...
	private function dbQuery($queryType, $oUserEnvironment, $oUser){
		try{
			error_log('(136) queryType sent to user database_integrations class object :: '.$queryType);
...
//						';
				...
				case 'svc_resetPassword':
					#error_log("/services/database.inc.php (967) USERNAME: ".$oUser->getReqParamByKey('USERNAME')." EMAIL: ".$oUser->getReqParamByKey('EMAIL'));
					...
					if(sizeof(self::$result_ARRAY[0])>1){
						...
						#error_log("/services/database.inc.php (1035) query: ".self::$query);
						//
						// PROCESS QUERY
						self::$result = $oUserEnvironment->oMYSQLI_CONN_MGR->processMultiQuery($mysqli, self::$query);
						if($mysqli->error){
								...
								if (self::$result = $mysqli->store_result()) {
									while ($row = self::$result->fetch_row()) {
										foreach($row as $fieldPos=>$value){
											//
											// STORE RESULT
											error_log("/services/database.inc.php (1013) ".$value);
											self::$result_ARRAY[$ROWCNT][$fieldPos]=$value;
										}
										$ROWCNT++;
									}
...
//						// ROLL BACK DUE TO ERROR
				...
				case 'svc_retrieveUserAccnt_PlusNav':
					...
					#error_log('/services/ database.inc.php (1769) :: svc_retrieveUserAccnt: '.self::$query);
					//
					// PROCESS QUERY
					self::$result = $oUserEnvironment->oMYSQLI_CONN_MGR->processMultiQuery($mysqli, self::$query);
					if($mysqli->error){
