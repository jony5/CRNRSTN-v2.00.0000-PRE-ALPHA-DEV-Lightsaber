...
class database_integration {
	...
	public function svc_updateMessage($oUser, $oUserEnvironment){
		return $this->dbQuery('svc_updateMessage', $oUserEnvironment, $oUser);
	}
	...
	private function dbQuery($queryType, $oUserEnvironment, $oUser){
		try{
			error_log('(156) queryType sent to MGMT database_integrations class object :: '.$queryType);
			...
			switch($queryType){
				...
				case 'publishAccntNote':
					...
					switch($tmp_notecheck){
						case '0':
							...
							"'.$mysqli->real_escape_string(self::$result_ARRAY[0][5]).'",
							"'.$mysqli->real_escape_string(self::$result_ARRAY[0][6]).'",
							"'.$mysqli->real_escape_string(self::$result_ARRAY[0][7]).'",
						...
						case '1':
							...
							`'.$dyn_tbl_notes_publish.'`.`SUBJECT`="'.$mysqli->real_escape_string(self::$result_ARRAY[0][4]).'",
							`'.$dyn_tbl_notes_publish.'`.`NOTE_STYLED`="'.$mysqli->real_escape_string(self::$result_ARRAY[0][5]).'",
							`'.$dyn_tbl_notes_publish.'`.`NOTE_RAW`="'.$mysqli->real_escape_string(self::$result_ARRAY[0][6]).'",
							`'.$dyn_tbl_notes_publish.'`.`NOTE_ELEM_TT`="'.$mysqli->real_escape_string(self::$result_ARRAY[0][7]).'",
					...
					#error_log('/services/database.mgmt.inc.php (1654) publishnote :: '.self::$query);
				...
				case 'svc_submitNewMessage':
					//
					// BUILD QUERY
					self::$query = 'INSERT INTO `msg_content` (`MSG_KEYID`,`MSG_KEYID_CRC32`,`MSG_NAME`,`MSG_SUBJECT`,`MSG_HTML`,`MSG_TEXT`,`MSG_DESCRIPTION`,`DATEMODIFIED`) VALUES 
					("'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_KEYID')).'","'.$oUser->getReqParamByKey('MSG_KEYID_CRC32').'","'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_NAME')).'","'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_SUBJECT')).'","'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_HTML')).'","'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_TEXT')).'","'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_DESCRIPTION')).'","'.$ts.'");';
				...
				break;
				case 'svc_updateMessage':
					#error_log("/services/ database.mgmt.inc.php (1819) svc_updateMessage");
					self::$query = 'UPDATE `msg_content` SET `MSG_NAME`="'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_NAME')).'",`MSG_SUBJECT`="'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_SUBJECT')).'",`MSG_HTML`="'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_HTML')).'",`MSG_TEXT`="'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_TEXT')).'",`MSG_DESCRIPTION`="'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_DESCRIPTION')).'",`DATEMODIFIED`="'.$ts.'" 
					WHERE `MSG_KEYID`="'.$mysqli->real_escape_string($oUser->getReqParamByKey('MSG_KEYID')).'" AND `MSG_KEYID_CRC32`="'.$oUser->getReqParamByKey('MSG_KEYID_CRC32').'" LIMIT 1;';
					...
					if($mysqli->error){
						self::$query_exception_result='editmessage=false';	
						throw new Exception('CRNRSTN database_integration :: '.$queryType.' ERROR :: ['.$mysqli->error.'] on un='.strtolower($oUser->getReqParamByKey('USERNAME')));
					}
				...
				case 'svc_retrieveMessages':
					//
					// BUILD QUERY
					self::$query = 'SELECT `msg_content`.`MSG_KEYID`,`msg_content`.`ISACTIVE`,`msg_content`.`LANGCODE`,`msg_content`.`MSG_NAME`,`msg_content`.`MSG_SUBJECT`,`msg_content`.`MSG_HTML`,`msg_content`.`MSG_TEXT`,`msg_content`.`MSG_DESCRIPTION`,`msg_content`.`DATEMODIFIED`,`msg_content`.`DATECREATED` FROM `msg_content` WHERE `msg_content`.`ISACTIVE`="1" OR `msg_content`.`ISACTIVE`="3";';
