...
#$oCRNRSTN->grantExclusiveAccess('LOCALHOST_MAC','127.0.0.1');
...
#error_log("_crnrstn.config.inc.php (236) About to check for session config initialization...");
$oENV = new crnrstn_environmentals($oCRNRSTN,'simple_configcheck');
if(!$oENV->isConfigured($oCRNRSTN)){
#	if(2==2){
	#error_log('_crnrstn.config.inc.php (239) :: I AM NOT CONFIGURED IN SESSION. DEFINING RESOURCE KEYS NEXT.');
	unset($oENV);
	//
...
require($ROOT.'/common/classes/user.inc.php');									// CUSTOM USER CLASS
require($ROOT.'/common/classes/database.inc.php');								// CUSTOM DATABASE CLASS
## ##
//
// SESSION TEST
