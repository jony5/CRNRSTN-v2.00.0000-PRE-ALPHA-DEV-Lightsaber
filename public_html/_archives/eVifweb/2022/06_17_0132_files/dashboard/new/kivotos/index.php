...
if(strlen($tmp_cid)!=50){
    ...
    echo "<br>(30) We need this...die()";
    error_log("(30) We need this...die()");
    die();
	//
	...
	*/
    ...
    echo "<br>(89) We need this...die()";
    error_log("(89) We need this...die()");
    die();

	//
	// WE HAVE CLIENT. PULL LIST OF USERS APPROVED TO RECEIVE INITIAL ASSIGBNMENT UPON KIVOTOS CREATION
...
<div data-role="page">
	...
	<div role="main" class="ui-content">
		...
		if(strlen($tmp_cid)==50 && sizeof($tmp_clientData)>0){
		?>	
      	<form action="#" method="post" name="create_kivotos" id="create_kivotos"  enctype="multipart/form-data" data-ajax="false">				
            ...
            <select name="user_assign" id="user_assign">
				...
				for($i=0; $i<sizeof($tmp_userData); $i++){
					...
					if(isset($tmp_userClient[$tmp_userData[$i]['USERID']])){
						if(sizeof($tmp_userClient[$tmp_userData[$i]['USERID']])>0){
							for($ii=0;$ii<sizeof($tmp_userClient[$tmp_userData[$i]['USERID']]);$ii++){
								if(isset($tmp_userClient[$tmp_userData[$i]['USERID']][$tmp_cid]) && !isset($userDisplay[$tmp_userData[$i]['USERID']])){
									if($tmp_userData[$i]['USERID']==$oCRNRSTN_ENV->oSESSION_MGR->getSessionParam("USERID")){
										$tmpOutputHTML .= '<option value="'.$oCRNRSTN_ENV->paramTunnelEncrypt($tmp_userData[$i]['USERID']).'" selected="selected">'.$tmp_userData[$i]['FIRSTNAME_BLOB']." ".$tmp_userData[$i]['LASTNAME_BLOB'].'</option>'; 
									}else{
										$tmpOutputHTML .= '<option value="'.$oCRNRSTN_ENV->paramTunnelEncrypt($tmp_userData[$i]['USERID']).'">'.$tmp_userData[$i]['FIRSTNAME_BLOB']." ".$tmp_userData[$i]['LASTNAME_BLOB'].'</option>'; 
									}
