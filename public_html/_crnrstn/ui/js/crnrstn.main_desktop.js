...
(function (root, factory) {
    ...
    */
    ...
    CRNRSTN_JS.prototype.crnrstn_rtime_timer_cycle = function() {
        ...
        this.log_activity('[lnum 1020] Begin client cycling of a second. TTL delta=[' + this.ttl_age_seconds + ']', this.CRNRSTN_DEBUG_CONTROLS);
            ...
            if(this.ui_sync_controller_thread_delay_ARRAY[this.transaction_thread_id_key_ARRAY[i]] > -1 ){
                ...
                if(tmp_delay_secs > 0){
                    ...
                    if(tmp_delay_secs === 0){
                        ...
                        //this.log_activity('[lnum 1192] FIRE! FIRE! FIRE! Executing ' + this.transaction_thread_id_key_ARRAY[i] + ' UI sync now. Looking forward to hearing from you! All the best, J5.', this.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.process_data_tunnel_ttl = function() {
        ...
        this.log_activity('[lnum 1210] Analyzing SSDTLA TTL.', this.CRNRSTN_DEBUG_CONTROLS);
        ...
        if(this.ttl_age_seconds > this.data_tunnel_ttl_monitor_ARRAY['page_load_data_ttl'] && this.data_tunnel_ttl_monitor_ARRAY['page_load_data_ttl'] != -1){
            ...
            this.log_activity('[lnum 1216] CRNRSTN :: SOAP Services Data Tunnel Layer Architecture (SSDTLA) TTL EXPIRED - PAGE LOAD TTL (' + this.data_tunnel_ttl_monitor_ARRAY['page_load_data_ttl']  + ' secs).', this.CRNRSTN_DEBUG_VERBOSE);   // CRNRSTN_DEBUG_BASIC
            ...
            if(this.data_tunnel_ttl_monitor_isactive){
                ...
                if(this.thread_id_ttl_expired()){
                    ...
                    this.data_tunnel_ttl_monitor_isactive = false;
                    this.ttl_tunnel_monitor_seconds = 1;
                    this.log_activity('[lnum 1235] ***** fire_dom_state_controller() DEV ABORTED ***** [CRNRSTN_JS.prototype.process_data_tunnel_ttl()].', this.CRNRSTN_DEBUG_VERBOSE);
                    //this.fire_dom_state_controller();
                ...
                if(this.ttl_tunnel_monitor_seconds > 300){
                    ...
                    this.data_tunnel_ttl_monitor_isactive = false;
                    this.ttl_tunnel_monitor_seconds = 1;
                    this.log_activity('[lnum 1249] CRNRSTN :: SOAP Services Data Tunnel Layer Architecture (SSDTLA) TTL EXPIRED - INACTIVITY REFRESH TTL (5 min).', this.CRNRSTN_DEBUG_VERBOSE);   // CRNRSTN_DEBUG_BASIC
                    this.fire_dom_state_controller();
    ...
    CRNRSTN_JS.prototype.fire_dom_state_controller = function() {
        ...
        if(ssdtl_endpoint !== undefined && ssdtl_endpoint != ''){
            ...
            this.log_activity('[lnum 1275] Sending CRNRSTN :: SOAP Services Data Tunnel Layer Packet (SSDTLP) in AJAX POST to [' + ssdtl_endpoint + '].', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
            this.log_activity('[lnum 1276] SSDTLP Serialization Key = [' + $('#' + this.form_input_serialization_key).val() + '].', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
            this.log_activity('[lnum 1277] SSDTLP ' + this.system_hash_algo + ' Hash = [' + $('#' + this.form_input_serialization_hash).val() + '].', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);

            if($('#crnrstn_interact_ui_link_text_click').val() != ''){

                this.log_activity('[lnum 1281] SSDTLP [ACTION] = [' + $('#crnrstn_interact_ui_link_text_click').val() + '].', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
        ...
        //this.log_activity('[lnum 1313] CRNRSTN :: SOAP Services Data Tunnel Layer Architecture (SSDTLA) aborting AJAX POST to [' + ssdtl_endpoint + '].', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
    ...
    CRNRSTN_JS.prototype.parse_data_tunnel_response_html = function(response_data) {
        ...
        oCRNRSTN_JS.log_activity('[lnum 1321] Receiving ' + oCRNRSTN_JS.pretty_format_number(packet_dl_bytes) + ' chars in POST response from CRNRSTN :: SOAP Services Data Tunnel Layer Architecture (SSDTLA).', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.parse_data_tunnel_response = function(response_data) {
        ...
        //var packet_dl_bytes = response_data.documentElement.innerHTML.length;
        var packet_dl_bytes = response_data.length;
        oCRNRSTN_JS.log_activity('[lnum 1337] Receiving ' + oCRNRSTN_JS.pretty_format_number(packet_dl_bytes) + ' chars in POST response from CRNRSTN :: SOAP Services Data Tunnel Layer Architecture (SSDTLA).', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.initialize_ttl_tracking = function(nomination, index) {
        ...
        */
        ...
        if($.inArray(nomination, this.xml_node_ttl_indices_ARRAY) >= 0){
            ...
            this.log_activity('[lnum 1446] TTL tracking has been initialized for XML node [' + nomination + '] with index append of [\'_' + index + '\'] .', this.CRNRSTN_DEBUG_BASIC);
        ...
        switch(nomination){
            case 'banner_img_uri':
                ...
                if(!(response_data in this.jony5_lifestyle_banner_index_ARRAY)){
                    ...
                    var tmp_str = this.extract_filename(response_data);
                    this.log_activity('[lnum 1468] SKIPPING CRNRSTN :: SSDTL RESPONSE REDUNDANT XML DATA [' + tmp_str + '] FROM NODE ' + nomination + '.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
            ...
            case 'banner_img_full_scrn_uri':
                ...
                if(!(response_data in this.jony5_lifestyle_banner_index_FULLSCREEN_ARRAY)){
                    ...
                    var tmp_str = this.extract_filename(response_data);
                    this.log_activity('[lnum 1480] STORING CRNRSTN :: SSDTL RESPONSE XML DATA [' + tmp_str + '] FROM NODE ' + nomination + '.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                    ...
                    var tmp_str = this.extract_filename(response_data);
                    this.log_activity('[lnum 1488] SKIPPING REDUNDANT CRNRSTN :: SSDTL RESPONSE XML DATA [' + tmp_str + '] FROM NODE ' + nomination + '.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
    ...
    CRNRSTN_JS.prototype.consume_data_tunnel_xml_node = function(response_data, nomination, xml_nom, node_attribute_nom, serialize_response, index = 0) {
        ...
        if(node_attribute_nom.length > 0){
            ...
            this.log_activity('[lnum 1531] Storing XML data (len='+ tmp_data.length +') attribute [' + node_attribute_nom + '] from XML node [' + xml_nom + '] as [' + nomination + '] with index append of [\'_' + index + '\'] .', this.CRNRSTN_DEBUG_CONTROLS);

        }else{

            this.log_activity('[lnum 1535] Storing XML node [' + xml_nom + '] data[' + tmp_data + '] (len='+ tmp_data.length +') as [' + nomination + '] with index append of [\'_' + index + '\'] .', this.CRNRSTN_DEBUG_CONTROLS);
        ...
        switch(data_type){
            case 'XML':
                ...
                if (NODE_client_response.length > 0) {
                    ...
                    this.log_activity('[lnum 2111] Extracting ' + data_type + ' data from CRNRSTN :: SOAP Services Data Tunnel Layer (SSDTL) response.', this.CRNRSTN_DEBUG_VERBOSE);
                    ...
                    if (NODE_state_synchronization_data.length > 0) {
                        ...
                        this.log_activity('[lnum 2144] Extracting [state_synchronization_data] data from CRNRSTN :: SSDTL response.', this.CRNRSTN_DEBUG_CONTROLS);
                    ...
                    if (tmp_node_cnt > 0) {
                        ...
                        for(let i = 0; i < tmp_node_cnt; i++){
                            ...
                            this.log_activity('[lnum 2158] Extracting [response_status] data from CRNRSTN :: SSDTL response.', this.CRNRSTN_DEBUG_CONTROLS);
                    ...
                    if (tmp_node_cnt > 0) {
                        ...
                        for(let i = 0; i < tmp_node_cnt; i++){
                            ...
                            this.log_activity('[lnum 2174] Extracting [client_profile] data from CRNRSTN :: SSDTL response.', this.CRNRSTN_DEBUG_CONTROLS);
                    ...
                    if (tmp_node_cnt > 0) {
                        ...
                        for(let i = 0; i < tmp_node_cnt; i++){
                            ...
                            this.log_activity('[lnum 2190] Extracting [crnrstn_ui_interact_profile] data from CRNRSTN :: SSDTL response.', this.CRNRSTN_DEBUG_VERBOSE);
                    ...
                    if (tmp_node_cnt > 0) {
                        ...
                        for(let i = 0; i < tmp_node_cnt; i++){
                            ...
                            this.log_activity('[lnum 2206] Extracting [bassdrive] data from CRNRSTN :: SSDTL response.', this.CRNRSTN_DEBUG_CONTROLS);
                    ...
                    if (tmp_node_cnt > 0) {
                        ...
                        for(let i = 0; i < tmp_node_cnt; i++){
                            ...
                            this.log_activity('[lnum 2222] Extracting [lifestyle_banner] data from CRNRSTN :: SSDTL response.', this.CRNRSTN_DEBUG_CONTROLS);
                    ...
                    this.log_activity('[lnum 2231] CRNRSTN :: SSDTL response consumption is now complete.', this.CRNRSTN_DEBUG_VERBOSE);
                    this.log_activity('[lnum 2232] CRNRSTN :: SSDTL TTL monitoring for client state is now active.', this.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.thread_id_ttl_expired = function() {
        ...
        for(let i = 0; i < tmp_ttl_cnt; i++){
            ...
            if((tmp_array_adjusted_ttl < this.ttl_age_seconds) && (tmp_array_adjusted_ttl > 0)){
                ...
                this.log_activity('[lnum 2423] TTL for a data tunneled thread [' + this.ttl_array_pointer_index_ARRAY[i] + '] has been expired at ' + this.ttl_age_seconds + ' seconds, where the threshold is ' + tmp_array_adjusted_ttl + ' secs.', this.CRNRSTN_DEBUG_BASIC);
    ...
    CRNRSTN_JS.prototype.update_thread_count = function(thread_id, delta, scheduler_invoked) {
        ...
        if(!scheduler_invoked){
            ...
            this.log_activity('[lnum 2449] UI sync controller transaction thread count updated to ' + tmp_cnt + ' with the initialization of [' + thread_id + '].', this.CRNRSTN_DEBUG_CONTROLS);
        ...
        switch(ui_thread_id){
            ...
            case 'lifestyle_banner_image_rotation':
                ...
                if(this.jony5_banner_mode == 'PLAY'){
                    ...
                    this.log_activity('[lnum 3020] FIRE! FIRE! FIRE! [' + ui_thread_id + ']. ', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
            ...
            default:
                ...
                this.log_activity('[lnum 3029] UI sync instructions have not been configured for [' + ui_thread_id + '].', this.CRNRSTN_DEBUG_CONTROLS);
    ...
    CRNRSTN_JS.prototype.execution_delay_ui_sync_controller = function(action_init, action_id, delay_ttl = 0, scheduler_invoked = false){
        ...
        switch(action_init){
            case 'fire':
                ...
                this.log_activity('[lnum 3044] UI sync controller set to fire [' + action_id + '].', this.CRNRSTN_DEBUG_CONTROLS);
            ...
            case 'schedule':
                ...
                this.log_activity('[lnum 3057] UI sync controller has scheduled [' + action_id + '] to fire in ' + delay_ttl + ' seconds.', this.CRNRSTN_DEBUG_UI);
            ...
            case 'cancel':
                ...
                this.log_activity('[lnum 3067] UI sync controller has cancelled [' + action_id + '].', this.CRNRSTN_DEBUG_CONTROLS);

            break;
            case 'sleep':

                this.log_activity('[lnum 3072] UI sync controller has put [' + action_id + '] to sleep.', this.CRNRSTN_DEBUG_CONTROLS);

            break;
            case 'silence_is_golden':

                //this.update_thread_count(action_id, 1, scheduler_invoked);
                this.log_activity('[lnum 3078] UI sync controller has scheduled [' + action_id + '] to run silently in ' + delay_ttl + ' seconds.', this.CRNRSTN_DEBUG_CONTROLS);
    ...
    CRNRSTN_JS.prototype.initialize_jony5_lifestyle_banner_controller = function() {
        ...
        $('.jony5_lifestyle_image').each(function(i, obj) {
            ...
            if(!(image_uri in self.jony5_lifestyle_banner_index_ARRAY)){
                ...
                self.log_activity('[lnum 3101] STORING STATIC BANNER DOM DATA [' + tmp_str + '].', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
        ...
        this.log_activity('[lnum 3121] STATIC DOM LOAD [' + tmp_str + '] INTO ALPHA, where Z=' + tmp_zindex_alpha + ' OPACITY=' + $('#jony5_banner_lifestyle_alpha').css('opacity') + '.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
        ...
        this.log_activity('[lnum 3137] STATIC DOM LOAD [' + tmp_str + '] INTO BETA, where Z=' + tmp_zindex_beta + ' OPACITY=' + $('#jony5_banner_lifestyle_beta').css('opacity') + '.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);

        this.load_image_lowest_z_indice('jony5_banner_lifestyle_alpha', 'jony5_banner_lifestyle_beta', tmp_next_image);

        this.log_activity('[lnum 3141] INITIALIZE 7 SECOND TTL ON ELEMENT HOLD - BANNER IMAGE ROTATION.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
    ...
    CRNRSTN_JS.prototype.size_element_y = function(dom_elem_id) {
        ...
        switch(dom_elem_id){
            ...
            default:
                ...
                this.log_activity('[lnum 3347] Unknown DOM element id [' + dom_elem_id + '] received by size_element(). No action taken.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
    ...
    CRNRSTN_JS.prototype.size_element = function(dom_elem_id) {
        ...
        switch(dom_elem_id){
            ...
            default:
                ...
                this.log_activity('[lnum 3420] Unknown DOM element id [' + dom_elem_id + '] received by size_element(). No action taken.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
    ...
    */
    ...
    CRNRSTN_JS.prototype.rotate_jony5_lifestyle_image = function(dom_element_alpha, dom_element_beta){
        ...
        if(tmp_zindex_alpha < tmp_zindex_beta){
            ...
            this.log_activity('[lnum 4954] ALPHA Z-SHIFTED ' + tmp_zindex_alpha + '->' + tmp_zindex_beta + '. [' + tmp_str + '] NOW FIRING! OPACITY-TRANSITION ' + $('#' + dom_element_alpha).css( "opacity" ) + ' TO 1.0.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                        ...
                        complete: function () {
                            ...
                            if(self.jony5_lifestyle_banner_sequence_control_ARRAY.length > seq_adj_pos){
                                ...
                                if((result != 0) && (result < self.jony5_lifestyle_banner_sequence_control_ARRAY.length)){
                                    ...
                                    self.log_activity('[lnum 5000] VISIBILITY FWD BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);

                                    self.init_ui_input_control_state('forward_btn_banner', 'ON');

                                }else{

                                    self.log_activity('[lnum 5006] VISIBILITY FWD BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);
                                ...
                                if(self.jony5_lifestyle_banner_sequence_position > 2){
                                    ...
                                    self.log_activity('[lnum 5014] VISIBILITY BACK BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);

                                    self.init_ui_input_control_state('back_btn_banner', 'ON');

                                }else{

                                    self.log_activity('[lnum 5020] VISIBILITY BACK BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);
                                ...
                                self.log_activity('[lnum 5037] QUEUE RANDOM IMAGE ' + tmp_index + ' (OUT OF ' + self.jony5_lifestyle_banner_images_ARRAY.length + '). [' + tmp_str + '].', self.CRNRSTN_DEBUG_VERBOSE);
                                ...
                                self.log_activity('[lnum 5052] FWD BUTTON ON IF [' + self.back_press_cnt + '][' + self.jony5_lifestyle_banner_sequence_position + '][' + result + '] < [' + self.jony5_lifestyle_banner_sequence_control_ARRAY.length + ']', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                                ...
                                self.log_activity('[lnum 5066] INITIALIZE 7 SECOND TTL ON ELEMENT HOLD - BANNER IMAGE ROTATION.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
            ...
            var tmp_str = $('#' + dom_element_beta).html();
            tmp_str = this.extract_filename(tmp_str);
            this.log_activity('[lnum 5091] BETA Z-SHIFTED ' + tmp_zindex_beta + '->' + tmp_zindex_alpha + ' [' + tmp_str + '] NOW FIRING! OPACITY-TRANSITION ' + $('#' + dom_element_beta).css( "opacity" ) + ' TO 1.0.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                        ...
                        complete: function () {
                            ...
                            if(self.jony5_lifestyle_banner_sequence_control_ARRAY.length > seq_adj_pos){
                                ...
                                if((result != 0) && (result < self.jony5_lifestyle_banner_sequence_control_ARRAY.length)){
                                    ...
                                    self.log_activity('[lnum 5137] VISIBILITY FWD BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);

                                    self.init_ui_input_control_state('forward_btn_banner', 'ON');

                                }else{

                                    self.log_activity('[lnum 5143] VISIBILITY FWD BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);
                                ...
                                if(self.jony5_lifestyle_banner_sequence_position > 2){
                                    ...
                                    self.log_activity('[lnum 5151] VISIBILITY BACK BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);

                                    self.init_ui_input_control_state('back_btn_banner', 'ON');

                                }else{

                                    self.log_activity('[lnum 5157] VISIBILITY BACK BUTTON banner_sequence_position=[' + self.jony5_lifestyle_banner_sequence_position + ']', self.CRNRSTN_DEBUG_VERBOSE);
                                ...
                                var tmp_str = self.extract_filename(tmp_next_image);
                                self.log_activity('[lnum 5174] QUEUE RANDOM IMAGE ' + tmp_index + ' (OUT OF ' + self.jony5_lifestyle_banner_images_ARRAY.length + '). [' + tmp_str + '].', self.CRNRSTN_DEBUG_VERBOSE);
                                ...
                                self.back_press_cnt = self.back_press_cnt * 1;
                                var result = self.jony5_lifestyle_banner_sequence_control_ARRAY.length - self.jony5_lifestyle_banner_sequence_position - self.back_press_cnt;
                                self.log_activity('[lnum 5188] FWD BUTTON ON IF [' + self.back_press_cnt + '][' + self.jony5_lifestyle_banner_sequence_position + '][' + result + '] < [' + self.jony5_lifestyle_banner_sequence_control_ARRAY.length + ']', self.CRNRSTN_DEBUG_VERBOSE);
                                if((result != 0) && (result < self.jony5_lifestyle_banner_sequence_control_ARRAY.length)){
                                ...
                                self.log_activity('[lnum 5201] INITIALIZE 7 SECOND TTL ON ELEMENT HOLD - BANNER IMAGE ROTATION.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
    ...
    CRNRSTN_JS.prototype.transition_lifestyle_banner_elem = function(dom_element_alpha, dom_element_beta){
        ...
        this.log_activity('[lnum 5266] TRANSITION AFTER BUTTON PRESS. ALPHA-Z[' + tmp_zindex_alpha + '][' + tmp_alpha_img + '] *** BETA-Z[' + tmp_zindex_beta + '][' + tmp_beta_img + '].', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
        ...
        this.log_activity('[lnum 5273] NEW ALPHA-Z[' + tmp_zindex_alpha_new + '][' + tmp_alpha_img + '] *** BETA-Z[' + tmp_zindex_beta_new + '][' + tmp_beta_img + '].', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                ...
                complete: function () {
                    ...
                    $('#' + dom_element_beta).animate({
                        ...
                        complete: function () {
                            ...
                            self.log_activity('[lnum 5297] INITIALIZE 7 SECOND TTL ON ELEMENT HOLD - BANNER IMAGE ROTATION.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                            self.execution_delay_ui_sync_controller('schedule', 'lifestyle_banner_image_rotation', 7);
                ...
                complete: function () {
                    ...
                    $('#' + dom_element_alpha).animate({
                        ...
                        complete: function () {
                            ...
                            self.log_activity('[lnum 5330] INITIALIZE 7 SECOND TTL ON ELEMENT HOLD - BANNER IMAGE ROTATION.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                            self.execution_delay_ui_sync_controller('schedule', 'lifestyle_banner_image_rotation', 7);
        ...
        if(tmp_zindex_alpha < tmp_zindex_beta){
            ...
            $('#' + dom_element_alpha).animate({
                ...
                complete: function () {
                    ...
                    self.log_activity('[lnum 5369] LOAD [' + tmp_fname + '] INTO ALPHA, where Z=' + tmp_zindex_alpha + ' OPACITY=' + $('#' + dom_element_alpha).css('opacity') + '.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
            ...
            $('#' + dom_element_beta).animate({
                ...
                complete: function () {
                    ...
                    self.log_activity('[lnum 5388] LOAD [' + tmp_fname + '] INTO BETA, where Z=' + tmp_zindex_beta + ' OPACITY=' + $('#' + dom_element_beta).css('opacity') + '.', self.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
        ...
        switch(mode){
            case 'pause_play':
                ...
                if(this.jony5_banner_mode == 'PLAY'){
                    ...
                    this.log_activity('[lnum 5425] STOP BANNER IMAGE ROTATION.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                    this.execution_delay_ui_sync_controller('stop', 'lifestyle_banner_image_rotation');

                }else{

                    this.jony5_banner_mode = 'PLAY';
                    this.log_activity('[lnum 5431] INITIALIZE 7 SECOND TTL ON ELEMENT HOLD - BANNER IMAGE ROTATION.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                    this.execution_delay_ui_sync_controller('schedule', 'lifestyle_banner_image_rotation', 7);
            ...
            case 'forward':
                ...
                this.log_activity('[lnum 5441] ADVANCE ONE (1) BANNER IMAGE.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                this.jony5_lifestyle_advance_button('forward');

            break;
            case 'back':

                this.log_activity('[lnum 5447] REVERSE ONE (1) BANNER IMAGE.', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
    ...
    CRNRSTN_JS.prototype.jony5_lifestyle_banner_tmp_transition = function(){
        ...
        this.log_activity('[lnum 5482] PRE-TRANSITION Z-INDICES tmp_zindex_trans=[' + tmp_zindex_trans + '] tmp_zindex_alpha=[' + tmp_zindex_alpha + '] tmp_zindex_beta=[' + tmp_zindex_beta + ']', this.CRNRSTN_DEBUG_VERBOSE);
        ...
        if(tmp_zindex_alpha < tmp_zindex_beta){
            ...
            $( "#jony5_banner_lifestyle_beta").animate({
                ...
                complete: function () {
                    ...
                    self.log_activity('[lnum 5510] MID-TRANSITION Z-INDICES tmp_zindex_trans=[' + tmp_zindex_trans + '] tmp_zindex_alpha=[' + tmp_zindex_alpha + '] tmp_zindex_beta=[' + tmp_zindex_beta + ']', self.CRNRSTN_DEBUG_VERBOSE);
            ...
            $( "#jony5_banner_lifestyle_alpha").animate({
                ...
                complete: function () {
                    ...
                    self.log_activity('[lnum 5581] MID-TRANSITION Z-INDICES tmp_zindex_trans=[' + tmp_zindex_trans + '] tmp_zindex_alpha=[' + tmp_zindex_alpha + '] tmp_zindex_beta=[' + tmp_zindex_beta + ']', self.CRNRSTN_DEBUG_VERBOSE);
        ...
        this.log_activity('[lnum 5633] POST-TRANSITION Z-INDICES tmp_zindex_trans=[' + tmp_zindex_trans + '] tmp_zindex_alpha=[' + tmp_zindex_alpha + '] tmp_zindex_beta=[' + tmp_zindex_beta + ']', this.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.jony5_lifestyle_advance = function(mode, delta = 1){
        ...
        switch(mode){
            ...
            case 'back':
                ...
                // this.log_activity('[lnum 5675] BACK CLICKED. NEW (-3) SEQ POS=[' + this.jony5_lifestyle_banner_sequence_position + '] IMAGE AT SEQ POS[' + tmp_str + '] SEQ-IMAGE-CNT[' + this.jony5_lifestyle_banner_sequence_control_ARRAY.length + ']', this.CRNRSTN_DEBUG_LIFESTYLE_BANNER);
                //
        ...
        if((result != 0) && (result < this.jony5_lifestyle_banner_sequence_control_ARRAY.length)){
            ...
            this.log_activity('[lnum 5706] VISIBILITY FWD BUTTON banner_sequence_position=[' + this.jony5_lifestyle_banner_sequence_position + ']', this.CRNRSTN_DEBUG_VERBOSE);

            this.init_ui_input_control_state('forward_btn_banner', 'ON');

        }else{

            this.log_activity('[lnum 5712] VISIBILITY FWD BUTTON banner_sequence_position=[' + this.jony5_lifestyle_banner_sequence_position + ']', this.CRNRSTN_DEBUG_VERBOSE);
        ...
        if(this.jony5_lifestyle_banner_sequence_position > 2){
            ...
            this.log_activity('[lnum 5720] VISIBILITY BACK BUTTON banner_sequence_position=[' + this.jony5_lifestyle_banner_sequence_position + ']', this.CRNRSTN_DEBUG_VERBOSE);

            this.init_ui_input_control_state('back_btn_banner', 'ON');

        }else{

            this.log_activity('[lnum 5726] VISIBILITY BACK BUTTON banner_sequence_position=[' + this.jony5_lifestyle_banner_sequence_position + ']', this.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.jony5_lifestyle_banner_state_revert = function(delta){
        ...
        this.log_activity('[lnum 5753] BANNER STATE REVERSION DELTA[' + delta + '] index/img alpha=[' + index_img_seq_int_alpha + '/' + image_uri_alpha + '] index/img beta=[' + index_img_seq_int_beta + '/' + image_uri_beta + ']', this.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.sign_in_transition_via_micro_expansion = function(){
        ...
        this.log_activity('[lnum 5953] [MOUSE CLICK] CRNRSTN :: UI INTERACT MODULE BEGIN SIGN IN FORM EXPANSION', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.primary_ui_interact_nav_state_shift = function(dom_handle_root, dom_handle_variant_element, dom_handle_postfix, element_exclude = ''){
        ...
        */
        ...
        this.log_activity('[lnum 6149] zIndex shift for all ' + dom_handle_root + dom_handle_variant_element + '.', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
        ...
        for(let i = 0; i < tmp_state_cnt; i++){
            ...
            if(tmp_z_index > tmp_tgt_z){
                ...
                if(tmp_str.length > 0 || (element_exclude != dom_handle_root + dom_handle_variant_element)){
                    ...
                    this.log_activity('[lnum 6164] zIndex shift for HIGHEST AT 68 [' + dom_handle_root + dom_handle_variant_element + system_nav_state_ARRAY[i] + '].', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
                ...
                $('#' + dom_handle_root + dom_handle_variant_element + system_nav_state_ARRAY[i]).css('zIndex', tmp_tgt_z);
                this.log_activity('[lnum 6181] zIndex shift TO 66 FOR OTHERS [' + dom_handle_root + dom_handle_variant_element + system_nav_state_ARRAY[i] + '].', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
            ...
            this.log_activity('[lnum 6188] current_z_index_state_ARRAY[' + dom_handle_root + dom_handle_variant_element + system_nav_state_ARRAY[i] + '] = ' + $('#' + dom_handle_root + dom_handle_variant_element + system_nav_state_ARRAY[i]).css('zIndex') + '.', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.primary_ui_interact_transition_nav_state = function(dom_handle_root, dom_handle_variant_element, dom_handle_postfix){
        ...
        $('#' + dom_handle_root + dom_handle_variant_element + dom_handle_postfix).animate({
            ...
            complete: function () {
                ...
                self.log_activity('[lnum 6221] SHOW ' + '#' + dom_handle_root + dom_handle_variant_element + dom_handle_postfix, oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
                ...
                $('#' + dom_handle_root + dom_handle_variant_element + dom_handle_postfix).animate({
                    ...
                    complete: function () {
                        ...
                        self.log_activity('[lnum 6238] REMOVE dom_element_mouse_state_lock_ARRAY LOCK ON [' + '#' + dom_handle_root + dom_handle_variant_element + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
        ...
        */
        ...
        switch(elem.id){
            case 'crnrstn_interact_ui_full_doc_close':
                ...
                switch(ux_action){
                    case 'onmouseover':
                        ...
                        //this.log_activity('[lnum 6283] ' + ux_action + ' FIRED ON [' + '#' + elem.id + '].', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
                    ...
                    case 'onmouseout':
                        ...
                        //this.log_activity('[lnum 6291] ' + ux_action + ' FIRED ON [' + '#' + elem.id + '].', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
            ...
            default:
                ...
                this.log_activity('[lnum 6458] [ACTION=' + ux_action + '] CRNRSTN :: UI INTERACT UNKNOWN ELEMENT ID [' + elem.id + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
    ...
    CRNRSTN_JS.prototype.____crnrstn_ui_interact_ux = function(ux_action, elem){
        ...
        switch(element_id){
            ...
            default:
                ...
                this.log_activity('[lnum 7029] [ACTION=' + ux_action + '] CRNRSTN :: UI INTERACT UNKNOWN ELEMENT ID [' + elem.id + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
        ...
        this.log_activity('[lnum 7044] element_id_split_ARRAY[0]=' + element_id_split_ARRAY[0] + '] element_id_split_ARRAY[1]=' + element_id_split_ARRAY[1] + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);

        if(this.dom_element_mouse_state_lock_ARRAY[dom_handle_element] === 'undefined'){

            alert('[lnum 7048] dom_element_mouse_state_lock_ARRAY is undefined at ' + dom_handle_element + ' for ' + element_id + '.');
            ...
            if(this.dom_element_mouse_state_lock_ARRAY[dom_handle_element] !== 'ON'){
                ...
                switch(ux_action){
                    case 'onmouseover':
                        ...
                        this.log_activity('[lnum 7059] [ACTION=onmouseover on ' + element_id + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);

                        this.log_activity('[lnum 7061] LOCK ON FOR [' + element_id + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
                    ...
                    case 'onmouseout':
                        ...
                        this.log_activity('[lnum 7106] [ACTION=onmouseout on ' + element_id + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
                ...
                this.log_activity('[lnum 7144] [ACTION=' + ux_action + '] [target_dom_handle=' + target_dom_handle + '] CRNRSTN :: UI INTERACT (UX) ELEMENT ID [' + elem.id + ']', oCRNRSTN_JS.CRNRSTN_DEBUG_VERBOSE);
