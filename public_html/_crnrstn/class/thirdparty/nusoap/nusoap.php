...
class soap_transport_http extends nusoap_base
{
    ...
    function __construct($url, $curl_options = null, $use_curl = false)
    {
        ...
        $oCRNRSTN = new crnrstn(__FILE__, 'In Him.');
        $tmp_version_crnrstn = $oCRNRSTN->proper_version();

        $this->setHeader('User-Agent', $this->title . '/' . $this->version . ' (' . $rev[1] . ') ' . $tmp_version_crnrstn);
...
class nusoap_server extends nusoap_base
{
    ...
    function send_response()
    {
        ...
        preg_match('/\$Revisio' . 'n: ([^ ]+)/', $this->revision, $rev);
        $this->outgoing_headers[] = "X-SOAP-Server: $this->title/$this->version (" . $rev[1] . ") " . $tmp_version_crnrstn;
